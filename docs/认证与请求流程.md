...
**简体中文** | [English](authentication-and-request-flow.en.md)
...
sequenceDiagram
    ...
    S->>F: 监听 .scratch/ 与 docs/architecture/
...
所有任务依赖图均经过上述工作流路由生成 Archify 产物。页面在受限 iframe 中展示图的概览；图下任务索引使用快照数据打开主页面的工单详情，展开大图则读取同一受校验产物。

服务端监听 `.scratch/` 与 `docs/architecture/`，以 30 毫秒延迟合并变化通知。SSE 事件只表示“文件已变化”，不直接包含完整任务；浏览器收到事件后重新取快照。连接关闭时移除客户端；进程收到退出信号后关闭监听与连接。
...
| 浏览器持久凭据 | 正式前端不使用 localStorage/sessionStorage 保存凭据；视图、功能、筛选与所选工单通过 URL 查询参数保留，这些参数不是凭据 |
