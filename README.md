...
![开发任务视图：SDD 五阶段与实际开发工单](docs/images/SDD开发工单概览.png)

> 实际运行截图：本项目展示的 4 张实际开发票据已完成，验证与交付独立显示“待记录”。截图仅代表拍摄时的项目状态。
...
保留原有 SDD 五阶段布局。“架构设计门禁与中文架构阶段卡片”作为架构资料，只在 SDD 的“架构设计”卡片中展示，不再混入规格与边界、任务计划、实施明细、依赖流程图或任务列表。进度只统计实际开发工单。

![SDD 架构卡片](docs/images/SDD架构设计.png)

### 依赖流程图与任务列表

每个实际开发功能保留自己的横向 Archify 图和工单分组，可以独立收起、展开。当前项目仅展示 Matt Task View 的 4 张实际开发工单；图下索引仍可定位工单详情，状态筛选与原布局保持一致。

![实际开发依赖图与工单](docs/images/实际开发依赖与工单.png)

以上是 Codex 内嵌浏览器的网页截图，不含 Codex 应用外框。

### 从进度进入具体任务

顶部先展示当前进度与下一步，五个可折叠阶段保留规格、计划和实施上下文。通过依赖图定位任务后，可展开验收项、源文件位置、批准修订与受影响组件。

![任务详情：验收项、批准修订和受影响组件](docs/images/任务详情.png)
...
更多截图与逐项解读见 [界面导览](docs/界面导览.md)，包含架构生命周期、摘要绑定和完整工作台。
...
使用 Node.js 内置测试运行器；本次发布在 Node.js `v24.13.1` 下通过 105 项测试，覆盖任务图、Archify 适配与交付、架构门禁、生命周期、渲染、HTTP 路由、路径拒绝和 SSE。
