...
> 实际运行截图：本项目展示 4 张实际开发工单，均已完成；架构内容只从架构图入口查看，验证与交付独立显示“待记录”。截图仅代表拍摄时的项目状态。
...
顶部“架构设计”入口只展示系统架构图。“架构设计门禁”资料不再作为开发功能、工单或任务依赖图重复列出。开发区域只显示实际功能的开发工单。
...
| 工作概览 | 单个功能的工单 |
| --- | --- |
| ![完成工单收进历史](docs/images/当前工作概览.png) | ![实际功能的四张工单](docs/images/历史功能工单.png) |
...
架构参考资料可在 `spec.md` 首个二级标题之前显式写入 `**View:** architecture`。该标记仅控制界面分类：资料只在架构图入口展示，不进入开发功能选择、进度、历史工单或任务依赖图。原始票据、API 快照和依赖／架构校验保留，其他功能不会仅因名称含“架构”而被隐藏。
...
使用 Node.js 内置测试运行器；本次发布在 Node.js `v24.13.1` 下通过 110 项测试，覆盖任务图、Archify 适配与交付、架构门禁、生命周期、渲染、HTTP 路由、路径拒绝和 SSE。
